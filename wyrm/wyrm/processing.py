...
def select_channels(dat, regexp_list, invert=False, chanaxis=-1):
    ...
    for c_idx, c in enumerate(dat.axes[chanaxis]):
        for regexp in regexp_list:
            # fix - LOCALE doesn't play nice with ascii
            if sys.version_info >= (3,0):
                # if python version > 3(.6, actually)
                m = re.match(regexp, c, re.IGNORECASE | re.ASCII)
            else:
                # do it the OLD way, since python2 doens't know about ASCII.
